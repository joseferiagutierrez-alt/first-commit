...
import { Terminal, Code2, CheckCircle2, AlertCircle, ArrowLeft } from "lucide-react";
...
export default function TestPage() {
  ...
  const [terminalStep, setTerminalStep] = useState(0); // 0: ls, 1: chmod, 2: ps
  ...
  const handleCommand = (e: React.FormEvent) => {
    ...
    if (terminalStep === 0) {
      if (cmd === 'ls -la' || cmd === 'ls -a' || cmd === 'll') {
        ...
        newHistory.push("Next Challenge: Make 'script.sh' executable for everyone.");
      } else {
        response = "Try listing ALL files (including hidden ones).";
        newHistory.push(response);
      }
    } else if (terminalStep === 1) {
      if (cmd.startsWith('chmod +x') || cmd.startsWith('chmod 777') || cmd.startsWith('chmod 755')) {
        response = "[SUCCESS] Permissions updated.\nNext Challenge: Check running processes.";
        nextStep = 2;
        newHistory.push(response);
      } else {
        response = "Command not recognized or incorrect flags for making executable.";
        newHistory.push(response);
      }
  ...
  const handleQuizSubmit = () => {
    ...
    const passed = score >= 3; // 60% passing
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="p-4 bg-gray-800 text-sm text-gray-400">
               Current Task: {
                 ...
                 terminalStep === 1 ? "Make script.sh executable" :
